...
class User(UserMixin,db.Model):
    ...
    def gravatar(self, size=100, default='identicon', rating='g'):
        ...
        hashtext = self.avatar_hash or hashlib.md5(self.email.encode('utf-8')).hexdigest()
        return '{url}/{hash}?s={size}&d={default}&r={rating}'.format(
            url=url, hash=hashtext, size=size, default=default, rating=rating)
...
class Photo(db.Model):
    ...
    title = db.Column(db.String(255))
    about = db.Column(db.Text)
    url = db.Column(db.String(255))  # 原图url
    url_s = db.Column(db.String(255))  # 展示图url
    url_t = db.Column(db.String(255))  # 缩略图url
    timestamp = db.Column(db.DateTime(), index=True, default=datetime.utcnow)
    author_id = db.Column(db.Integer, db.ForeignKey('users.id'))
